...
- A Supabase project (free plan) with **custom SMTP configured** — the built-in
  sender only delivers to project team members and caps at 2 emails/hour, which
  cannot support magic-link sign-in (research §4)
...
### 2. No anonymous access — US1 scenario 2, FR-001
...
### 3. A full block offline — US3, SC-002
...
### 4. Sync survives interruption — SC-005
...
### 5. Both swipers walk the same path — US2 scenario 5, SC-004
...
### 6. Each account gets its own deck — US2 scenario 6, SC-004
...
### 7. Deck ordering is faithful to the current app — FR-014, US2 scenario 4
...
### 8. Rate cap protects the free tier — SC-012, FR-032
...
### 9. Cold start is invisible — FR-030, SC-001
...
### 10. Deploy from the runbook — US5, SC-009, SC-010
...
### 11. The database is still there next month — research §2

Confirm the daily keep-alive workflow is enabled and green. **Expect**: `GET
/health` succeeding daily.
...
it is what stops the database from being deleted during a quiet month. Note the
failure mode the runbook must record: GitHub disables scheduled workflows in
repositories inactive for 60 days, so a repo that goes quiet loses its heartbeat
first and its database a week later.
