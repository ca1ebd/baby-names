...
- Hosted on Azure Static Web Apps (Free tier), resource group `baby-names-rg`, East US 2. Live URL: `https://polite-bay-0b309e70f.7.azurestaticapps.net` — no custom domain yet, intentionally deferred.
- Deploys via `.github/workflows/azure-static-web-apps.yml` — this is a **hand-written** workflow (checkout → npm ci → write `public/version.json` with `github.sha` → `npm run build` with `VITE_COMMIT_SHA` env → deploy with `skip_app_build: true`), not Azure's auto-generated one. That's intentional: it's what lets us bake the commit SHA into the bundle for the update-check feature and the About screen's build stamp.
- Every push to `main` auto-deploys. There's no staging environment for this project (unlike `contraction-timer`, which this app borrows conventions from).
...
- `RAW` holds four flat arrays: `girl.c`/`girl.u` and `boy.c`/`boy.u` (`c` = classic/traditional style, `u` = unisex/modern style — this tag isn't currently used for anything visual, it's just organizational). Each name gets a `g: "girl"|"boy"` tag used for the card's color band.
- All four arrays are validated to exclude: names starting with "D", and names ending in "y"/"ie"/"ey" — a constraint from the user's actual last name pairing badly with those endings. Any future name additions should keep following that rule unless told otherwise.
- Girl and boy pools are deliberately built with **zero name overlap** (checked at generation time) — this matters because in "both" mode the pools get concatenated, and picks are keyed by name string alone, so a collision would let picking a name in one gender silently affect the other.
- `GIRL_NAMES`/`BOY_NAMES`/`BOTH_NAMES` are shuffled once at module load with a **fixed seed** (`shuffled()`, seed `20260730`) — this is why both parents see cards in the same order regardless of device: the shuffle is deterministic, not `Math.random()`.
