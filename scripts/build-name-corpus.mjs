#!/usr/bin/env node
// Builds src/lib/nameCorpus.ts from public SSA baby-name data.
// Run manually (npm run corpus:build) — never in CI. The generated module is
// committed; the downloaded archive is not.
//
...
function curate(rows, { since, limit }) {
  const allTime = new Map(); // name -> { F, M }
  const recent = new Map(); // name -> { F, M }
  ...
  for (const [name, counts] of allTime) {
    ...
    const recentCounts = recent.get(name) ?? { F: 0, M: 0 };
    const rank = isGirl ? recentCounts.F : recentCounts.M;
    const tieBreak = isGirl ? counts.F : counts.M;
    (isGirl ? girl : boy).push({ name, rank, tieBreak });
  }

  const byPopularity = (a, b) => b.rank - a.rank || b.tieBreak - a.tieBreak || a.name.localeCompare(b.name);
  girl.sort(byPopularity);
  boy.sort(byPopularity);

  const cut = (list) => (limit ? list.slice(0, limit) : list).map((entry) => entry.name);
  return { girl: cut(girl), boy: cut(boy), dropped };
}
...
function renderModule({ girl, boy, sourceId, since, limit }) {
  const packed = (list) => JSON.stringify(list.join(","));
  return `// Generated by scripts/build-name-corpus.mjs — do not edit by hand.
// Source: ${sourceId}
// Generated: ${new Date().toISOString().slice(0, 10)} | Ranked by births since ${since}${limit ? ` | Cut: top ${limit}/gender` : " | No popularity cut"}
// Counts: ${girl.length} girl / ${boy.length} boy (${girl.length + boy.length} total)
//
// Packed as delimited strings rather than array literals: this parses far
// faster than a ~100k-element array literal and drops the per-name quote bytes.
// Ordered by descending popularity — index is the popularity rank.
...
async function main() {
  ...
  console.log(`\n  building name corpus (source: ${opts.source}, since: ${opts.since}${opts.limit ? `, limit: ${opts.limit}/gender` : ", no cut"})`);

  const { rows, sourceId } =
    opts.source === "ssa" ? await loadFromSsa(opts.input) : await loadFromMirror();

  const { girl, boy, dropped } = curate(rows, opts);
  assertInvariants(girl, boy);

  await mkdir(path.dirname(opts.out), { recursive: true });
  await writeFile(opts.out, renderModule({ girl, boy, sourceId, since: opts.since, limit: opts.limit }));

  const bytes = Buffer.byteLength(renderModule({ girl, boy, sourceId, since: opts.since, limit: opts.limit }));
  console.log(`
  girl:    ${girl.length.toLocaleString()}
  boy:     ${boy.length.toLocaleString()}
  total:   ${(girl.length + boy.length).toLocaleString()}
  dropped: ${dropped.invalid.toLocaleString()} (failed name-format check)
  wrote:   ${path.relative(ROOT, opts.out)} (${(bytes / 1024).toFixed(0)} KB raw)

  head girl: ${girl.slice(0, 6).join(", ")}
  head boy:  ${boy.slice(0, 6).join(", ")}
`);
}
